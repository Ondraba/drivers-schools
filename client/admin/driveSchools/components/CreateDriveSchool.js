...
class CreateDriveSchool extends Component {
  ...
  submit({ title, perex, content }) {
    ...
    this.props
      ...
      .then(() => {
        Router.push("/headquarter/driveSchools");
      });
  }
...
const mutation = gql`
  mutation CreateDriveSchool($title: String!, $perex: String!, $content: String!) {
    createDriveSchool(title: $title, perex: $perex, content: $content) {
      _id
      title
      perex
      content
